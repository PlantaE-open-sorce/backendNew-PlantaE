...
public class SensorReadingRepositoryJpa implements SensorReadingRepository {
        ...
        public PagedResult<SensorReading> search(SensorReadingSearchCriteria criteria) {
                ...
                return new PagedResult<>(
                                page.map(entity -> new SensorReading(
                                                com.ecotech.plantae.sensor.domain.valueobjects.SensorId
                                                                .of(entity.getSensorId()),
                                                entity.getTimestamp(),
                                                com.ecotech.plantae.sensor.domain.valueobjects.SensorMetric
                                                                .from(entity.getMetric()),
                                                entity.getValue(),
                                                entity.getQuality())).toList(),
        ...
        @Override
        public List<SensorActivity> mostActive(Instant from, Instant to, int top) {
                Pageable pageable = PageRequest.of(0, top);
                return springDataRepository.topActive(from, to, pageable)
                                .stream()
                                .map(result -> new SensorActivity(
                                                (String) result[0],
                                                (Long) result[1]))
                                .toList();
        }
